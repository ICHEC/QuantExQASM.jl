...
function register_gate(circ::Circuit.Circ, U::Symbol, gate::Matrix{<:Number})
...
function apply_cu!(c::Circuit.Circ, ctrl, tgt, reg, gl::GateOps.GateLabel)
    if String(gl.label)[end] == "x"
        ...
        Circuit.add_gatecall!(c, GateOps.c_u(gl, ctrl, tgt, reg))
        Circuit.add_gatecall!(c, GateOps.hadamard(tgt, reg))
...
=#
        ...
        apply_cx!(cct, q_ctrl[1], q_ctrl[2])
...
=#
        ...
        apply_cx!(cct, q_ctrl[2], q_ctrl[3])
        apply_cu!(cct, q_ctrl[3], q_tgt, assu)
        apply_cx!(cct ,q_ctrl[1], q_ctrl[3])
    ...
    elseif (length(q_ctrl)>=5) && (length(q_aux)>=length(q_ctrl)-2) && (U == GateOps.GateLabel(:x))
        append!(cct, apply_ncu([q_ctrl[end], q_aux[ 1 + length(q_ctrl)-3 ]], q_tgt, U) )
        
        for i in reverse(2:length(q_ctrl)-2)
            append!(cct, apply_ncu([q_ctrl[1 + i], q_aux[1 + (i-2)]], q_aux[1 + (i-1)], U) )
        end
