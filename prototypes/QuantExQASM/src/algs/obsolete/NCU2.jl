...
struct NCU
    cct::CircuitBuilder.Circuit
    n_ctrl::Int
    n_target::Int
    num_gates::Int
    gate_label::GateOps.GatLabel
end

function decompose_nc_gates(gate::GateOps.GateCallN)
    cct = CircuitBuilder.Circuit



    return NCU(cct, length(gate.ctrl), length(gate.target), length(gate.ctrl)+length(gate.target), gate.gate_label)
end


function apply_ncu(gate::GateOps.GateCallN, aux::Vector{Int}, call_depth::Int)
    # how many sqrt needed?

    for i in 1:length(gate.ctrl)
    # Does 
    if gate.gate_label in circuit.GateSet
        #pull out gate
    else
        # add to GateSet
    end
